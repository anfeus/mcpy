...
class CustomGrandCanonicalEnsemble(BaseEnsemble):
    def __init__(self,
                 atoms: Atoms,
                 cells: List[Cell],
                 units_type: str,
                 ...
                 minima_mode: str = 'a',
                 restart: bool = False,
                 check_status: str = 'check_status.json') -> None:
        """Grand-canonical Monte Carlo ensemble with simple JSON restart.

        When ``restart=True`` and ``traj_file``, ``outfile`` and
        ``check_status`` all exist, the last XYZ frame and saved random states
        are restored. The trajectory and outfile are then opened in append
        mode. In restart mode, ``run(steps)`` treats ``steps`` as the desired
        total number of completed GCMC steps.
        """
        self.restart = restart
        self.check_status = check_status
        self._restarting = self._restart_files_exist(
            restart=restart,
            traj_file=traj_file,
            outfile=outfile,
        ...
        # A valid restart must append, independently of the modes supplied by
        # the caller. A fresh run keeps the caller's original modes.
        if self._restarting:
            traj_mode = 'a'
            outfile_mode = 'a'
    ...
    @staticmethod
    def _restart_files_exist(restart: bool,
                             traj_file: Optional[str],
                             outfile: Optional[str],
                             check_status: str) -> bool:
        """Return True only for a complete, explicitly requested restart."""
        if not restart:
            return False
        if traj_file is None or outfile is None:
            raise ValueError("restart=True requires both traj_file and outfile")
        required = [traj_file, outfile, check_status]
        existing = [path for path in required if os.path.isfile(path)]
        if len(existing) == len(required):
            return True
        if existing:
            missing = [path for path in required if not os.path.isfile(path)]
            raise FileNotFoundError(
                "Incomplete restart data. Missing: " + ", ".join(missing)
            )
        # No restart files exist: restart=True starts a fresh calculation and
        # creates them normally.
        return False

    @staticmethod
    def _state_to_json(value: Any) -> Any:
        """Recursively convert tuples/NumPy scalars to JSON-safe objects."""
        if isinstance(value, tuple):
            return [CustomGrandCanonicalEnsemble._state_to_json(v) for v in value]
        if isinstance(value, list):
            return [CustomGrandCanonicalEnsemble._state_to_json(v) for v in value]
        if isinstance(value, dict):
            return {str(k): CustomGrandCanonicalEnsemble._state_to_json(v)
    ...
    def _state_from_json(value: Any) -> Any:
        """Convert JSON lists back to tuples for random.Random.setstate()."""
        if isinstance(value, list):
            return tuple(CustomGrandCanonicalEnsemble._state_from_json(v) for v in value)
        if isinstance(value, dict):
            return {k: CustomGrandCanonicalEnsemble._state_from_json(v)
                    for k, v in value.items()}
        return value

    @staticmethod
    def _get_rng_state(owner: Any) -> Optional[Any]:
        """Get the state of an mcpy RandomNumberGenerator, when present."""
        rng = getattr(owner, 'rng', owner)
        python_rng = getattr(rng, 'random', None)
        if python_rng is None or not hasattr(python_rng, 'getstate'):
            return None
        return python_rng.getstate()

    @staticmethod
    def _set_rng_state(owner: Any, state: Any) -> None:
        """Restore the state of an mcpy RandomNumberGenerator, when present."""
        rng = getattr(owner, 'rng', owner)
        python_rng = getattr(rng, 'random', None)
        if python_rng is not None and hasattr(python_rng, 'setstate'):
            python_rng.setstate(state)

    def _collect_random_states(self) -> Dict[str, Any]:
        """Collect acceptance, selector and individual move RNG states."""
        states = {
            'acceptance': self._get_rng_state(self.rng_acceptance),
            'move_selector': self._get_rng_state(self.move_selector),
            'moves': [],
        }
        for move in self.move_selector.move_list:
            states['moves'].append(self._get_rng_state(move))
        return states

    def _restore_random_states(self, states: Dict[str, Any]) -> None:
        acceptance_state = states.get('acceptance')
        if acceptance_state is not None:
            self._set_rng_state(self.rng_acceptance, acceptance_state)

        selector_state = states.get('move_selector')
        if selector_state is not None:
            self._set_rng_state(self.move_selector, selector_state)

        for move, state in zip(self.move_selector.move_list,
                               states.get('moves', [])):
            if state is not None:
                self._set_rng_state(move, state)

    def _save_check_status(self) -> None:
        """Atomically save restart information after a trajectory frame."""
        status = {
            'step': self._step,
            'last_trajectory_step': self._last_trajectory_step,
            'random_states': self._collect_random_states(),
            'exchange_attempts': self.exchange_attempts,
            'exchange_successes': self.exchange_successes,
            'move_counter_total': self.move_selector.move_counter_total,
            'move_acceptance_total': self.move_selector.move_acceptance_total,
            'move_failed_counter_total': self.move_selector.move_failed_counter_total,
        }
        json_status = self._state_to_json(status)
        temporary_file = self.check_status + '.tmp'
        try:
            with open(temporary_file, 'w', encoding='utf-8') as handle:
                json.dump(json_status, handle, indent=2)
                handle.flush()
                os.fsync(handle.fileno())
            os.replace(temporary_file, self.check_status)
        except OSError:
            self.logger.exception("Failed to write restart status '%s'",
                                  self.check_status)
            try:
    ...
    def _load_check_status(self) -> None:
        """Restore the last XYZ configuration, step, counters and RNGs."""
        try:
            with open(self.check_status, 'r', encoding='utf-8') as handle:
                status = json.load(handle)
        except (OSError, json.JSONDecodeError):
            self.logger.exception("Failed to read restart status '%s'",
                                  self.check_status)
            raise

        last_atoms = read(self._traj_file, index=-1, format='extxyz')
        self.atoms = last_atoms
        self.n_atoms = len(last_atoms)
        self.initial_atoms = len(last_atoms)
        self.E_old = self.compute_energy(last_atoms)
        self._step = int(status['step'])
        self._last_trajectory_step = int(
            status.get('last_trajectory_step', self._step)
        )
        self._last_logged_step = self._step
        self.exchange_attempts = int(status.get('exchange_attempts', 0))
        self.exchange_successes = int(status.get('exchange_successes', 0))

        for name in ('move_counter_total', 'move_acceptance_total',
                     'move_failed_counter_total'):
            if name in status:
                setattr(self.move_selector, name,
                        [int(v) for v in status[name]])

        random_states = self._state_from_json(status.get('random_states', {}))
    ...
    def initialize_run(self) -> None:
        """Open files and either initialize a fresh run or restore a restart."""
        if self._initialized:
            return

        if self._restarting:
            # Load before opening the append handles. This also verifies that
            # the trajectory and JSON checkpoint are readable.
            self._load_check_status()
        ...
        if self._restarting:
            self.logger.info(
                "GCMC restarting from step=%d: T=%s K, mu=%s, outfile=%s",
                self._step, self._temperature, self._mu, self._outfile,
            )
            return
        ...
        self._last_trajectory_step = 0
        self._save_check_status()
    ...
    def finalize_run(self) -> None:
        ...
        self.write_outfile()
        if self._last_trajectory_step != self._step:
            self.write_coordinates(self.atoms, self.E_old)
            self._last_trajectory_step = self._step
        self._save_check_status()
        self.logger.info(
    ...
    def _run(self) -> None:
        """One GCMC step plus interval-based writes."""
    ...
    def run(self, steps: int) -> None:
        """Run to ``steps`` total on restart, or run ``steps`` on a fresh run."""
        self.initialize_run()
        try:
            if self._restarting:
                remaining_steps = max(0, int(steps) - self._step)
            else:
